...
model.saveCreateMatchToDB=(createMatchTnfo)=>{
  const newMatch = {
    firstTeamName:createMatchTnfo.firstTeamName,  
  }

  const db=firebase.firestore();
  db.collection('Match').doc('VgMLdQtGMfsNBLSI97YP')
  .update({
    firstTeamName: firebase.firestore.FieldValue.String(newMatch),
  });
