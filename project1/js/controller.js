...
controller.validateCreateMatchForm = (createMatchTnfo)=>{
  const {firstTeamName,
    firstTeamLogo,
    secondTeamName,
    secondTeamLogo,} = createMatchTnfo;

    if(!firstTeamName){
      view.setMessage('firstTeam-error-message' , 'please input first team');
    } else {
      view.setMessage('firstTeam-error-message' , '');
    }

    if(!secondTeamName){
      view.setMessage('secondTeam-error-message' , 'please input first team');
    } else {
      view.setMessage('secondTeam-error-message' , '');
    }
    if (firstTeamName ){
      model.saveCreateMatchToDB(createMatchTnfo);
    }
